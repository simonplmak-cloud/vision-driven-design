...
api/                     ← Vercel MCP endpoint (deployed at vdd.simonmak.com).
  sse.js                 ← MCP SSE transport — GET returns SSE stream (or HTML for browsers), POST handles JSON-RPC 2.0 (initialize, tools/list, tools/call). 17 tools.
...
packages/                ← TypeScript monorepo.
  vdd-engine/            ← Shared core: 17 phase functions + Zod types.
  vdd-mcp/               ← MCP server: 17 tools, stdio + Streamable HTTP transports.
  vdd-cli/               ← CLI binary: 17 subcommands, --json mode.
